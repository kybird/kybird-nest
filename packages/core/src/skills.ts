...
import { REGRESS_STALL_THRESHOLD } from "@kybird/shared";
...
회귀하는 카드는 뭔가 설계가 잘못됐다는 신호다 — \`knest board\` 아래 회귀 요약이
${REGRESS_STALL_THRESHOLD}회 이상 회귀한 카드를 따로 경고해준다.

**검증에 실패했거나 요구사항이 틀렸으면 기각해라.** 그냥 \`mv\` 로 되돌리면
왜 튕겼는지가 안 남아서 받는 쪽이 같은 실수를 반복한다.

\`\`\`
knest reject <카드> "<사유>"            검증 실패 — 개발이 다시 받는다
knest reject <카드> "<사유>" --as dev   요구사항이 틀렸거나 불가능 — 기획이 받는다
\`\`\`

사유는 필수다. 받는 쪽은 그 한 줄만 보고 고치므로 구체적으로 써라.
...
**검증에 실패했거나 요구사항이 틀렸으면 기각해라** — 그냥 옮기면 왜 튕겼는지가
안 남는다. 사유는 필수다.
